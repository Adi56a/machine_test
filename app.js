...
app.post('/register',async (req,res) =>{
    ...
    if(userExist) return res.status(409).send('User already exist ')
     ...
     bcrypt.genSalt(10,(err,salt)=>{
        ...
        let token  = jwt.sign({email:email,userid:registerUser._id },process.env.JWT_SECRET);
...
app.post('/login' , async (req , res)=>{
    ...
    bcrypt.compare(password,userExist.password,function(err,result){
        if(result === true){
            let token  = jwt.sign({email:email,userid:userExist._id },process.env.JWT_SECRET);
             res.cookie("token",token)
